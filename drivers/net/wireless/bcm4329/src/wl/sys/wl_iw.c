  ...
  if (assoc_info.req_len) {
       ...
       WL_ASSOC(("\t"));
        for (i = 1, passoc_ie = extra; i <= req_ies_len ; i++) {
		    WL_ASSOC(("%02x ",*passoc_ie++));
		    if (!(i%16))
                    WL_ASSOC(("\n\t"));
        }
        WL_ASSOC(("\n"));

		passoc_ie = (uchar *)extra;
		passoc_ie_end = (uchar *)(extra + req_ies_len);
		while(passoc_ie < passoc_ie_end) {
			/* WL_ASSOC(("ID %02X, Len %d\n", passoc_ie[0], passoc_ie[1])); */
			if ((passoc_ie[0] == 0x30) && (passoc_ie[1] >= (22 + 16))) {
				WL_ASSOC(("PMKID:"));
				for (i=24; i<40; i++) {
					WL_ASSOC(("%02X ", passoc_ie[i]));
				}
				WL_ASSOC(("\n"));
			}
			passoc_ie += passoc_ie[1] + 2;
		}
...
static chanspec_t get_20M_chanspec(chanspec_t chanspec)
{
	chanspec_t ch = 0;

	int c = CHSPEC_CHANNEL(chanspec);
        if (c > 0) {
		if (CHSPEC_SB_UPPER(chanspec)) {
			ch = CH20MHZ_CHSPEC(c+2);
		} else if (CHSPEC_SB_LOWER(chanspec)) {
			ch = CH20MHZ_CHSPEC(c-2);
		} else {
			ch = chanspec;
		}
...
static int wl_iw_scan_cache_get_channels(chanspec_t *channels)
{
	...
	for(i=0; i<scan_cache_count; i++, pCache++)
	{
		if ((g_ssid.SSID_len == pCache->ssid_len)
			...
			chanspec_t *p;
			chanspec_t ch = get_20M_chanspec(pCache->chanspec);
			...
			if (p == pLast) {
				...
				if (c > 0) {
					*pLast++ = ch;
					cnt++;
					WL_DEBUG(("wl_iw_scan_cache_get_channels: %d %04X/%04X (%d/%d)\n", cnt, pCache->chanspec, ch, c, CHSPEC_CHANNEL(ch)));
				}
				else {
...
)
{
	...
	WL_ERROR(("%s return to WE %d bytes APs=%d\n", __FUNCTION__, dwrq->length, counter));
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	switch (event_type) {
			...
			WL_ERROR(("ROAM: %d/%d/%d Start %s scan, %d seconds, reason %u, %d channels\n",
				roamscan_new, roam->cache_valid, roam->cache_valid2,
						roamscan_new ? "new" :
			...
			WL_ERROR(("ROAM: Scan %d ms CH %d\n", ntoh32(e->reason), ntoh32(e->auth_type)));
		} else if (status == 6) {
			int i;
			int n = ntoh32(e->reason);
			int type = ntoh32(e->auth_type);
			chanspec_t *ch = (chanspec_t *)data;
			
			if (type == 0) {
				WL_ERROR(("ROAM: Scanning %d channels\n", n));
			} else if (type == 1) {
				WL_ERROR(("ROAM: Roam Cache Update %d channels\n", n));
			}

			for(i=0; i<n; i++) {
				WL_ERROR(("ROAM: 0x%04X\n", *ch++));
			}	
		} else if (status == 7) {
			   WL_ERROR(("ROAM: No join target %d\n", ntoh32(e->reason)));
		}
		break;
	case WLC_E_REASSOC:
		{
			uint8 *p = (uint8 *)data;
			int i;
			WL_ERROR(("Reassoc: %02X:%02X:%02X:%02X:%02X:%02X, Status %d\n",
				e->addr.octet[0], e->addr.octet[1], e->addr.octet[2], e->addr.octet[3], e->addr.octet[4], e->addr.octet[5], status));
			WL_ERROR(("ROAM: PMKID index(%d) %d, npmkid %d, %02X:%02X:%02X:%02X:%02X:%02X\nROAM: PMKID ",
				p[0]&0x80, p[0]&0x7F, p[1], p[2], p[3], p[4], p[5], p[6], p[7]));
			for(i=8; i<24; i++) WL_ERROR(("%02X ", p[i]));
			WL_ERROR(("\n"));
		}
		break;
#endif
	...
	case WLC_E_DISASSOC_IND:
		...
		WL_TRACE(("%s: status %d from %02X:%02X:%02X:%02X:%02X:%02X\n", 
			(event_type == WLC_E_DEAUTH_IND) ? "DEAUTH" : "DISASSOC", status, 
