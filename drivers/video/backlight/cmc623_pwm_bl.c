...
#define MAX_BACKLIGHT_VALUE_LCDPLS	1296  // KOR 1360 => 1296
...
static void cmc623_pwm_apply(struct platform_device *pdev, int level)
{
	...
	dev_dbg(&pdev->dev, "[CMC] %s:current_gamma_level=%d\n", __func__, current_gamma_level);
}
...
static void cmc623_pwm_apply_brightness(struct platform_device *pdev, int level)
{
	...
	dev_dbg(&pdev->dev, "[CMC] %s : apply_level=%d\n", __func__, level);
}
...
static void cmc623_pwm_send_intensity(struct backlight_device *bd)
{
	...
	if ( bd->props.power != FB_BLANK_UNBLANK ||
		 bd->props.fb_blank != FB_BLANK_UNBLANK || cmc623_pwm_suspended )
	{
		printk("[CMC] intensity :%d (current_intensity:%d)\n", intensity, current_intensity);
...
static int cmc623_pwm_check_fb(struct backlight_device *bd, struct fb_info *fi)
{
	if(bd->dev.parent->parent)
	{
		...
		{
			printk(KERN_INFO "[CMC] %s fb matched\n", __func__);
			return 1;
		}
	}

	printk(KERN_DEBUG "[CMC] %s fb unmatched\n", __func__);
	return 0;
}
...
static int cmc623_pwm_probe(struct platform_device *pdev)
{
	...
	printk("[CMC] cmc623_pwm Probe START!!!\n");
	...
	printk("[CMC] cmc623_pwm Probe END!!!\n");
	return 0;
}
