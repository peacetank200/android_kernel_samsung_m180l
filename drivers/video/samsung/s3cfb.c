...
#if defined(CONFIG_TARGET_LOCALE_KOR)
static int s3cfb_sysfs_show_pclk_mode(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", current_band_for_pclk);
}

static int s3cfb_sysfs_store_pclk_mode(struct device *dev, struct device_attribute *attr,  const char *buf, size_t len)
{
	// struct s3c_platform_fb *pdata = to_fb_plat(dev);
...
static DEVICE_ATTR(pclk_mode, 0664, s3cfb_sysfs_show_pclk_mode, s3cfb_sysfs_store_pclk_mode);
...
static void s3cfb_change_pclk_mode(int id, struct s3cfb_global *fbdev)
{
	struct s3cfb_global fb_struct;
...
static int __devinit s3cfb_probe(struct platform_device *pdev)
{
	...
	dev_info(fbdev->dev, "[LCD] s3cfb_probe :: registered successfully\n");
...
void s3cfb_early_suspend(struct early_suspend *h)
{
	...
	pr_debug("[LCD] s3cfb_early_suspend is called\n");
...
void s3cfb_late_resume(struct early_suspend *h)
{
	...
	pr_info("[LCD] s3cfb_late_resume is called\n");
	...
	dev_dbg(fbdev->dev, "[LCD] wake up from suspend\n");
	if (pdata->cfg_gpio)
		pdata->cfg_gpio(pdev);
...
#if defined(CONFIG_TARGET_LOCALE_KOR)
	// printk("[LCD] %s, current_band_for_pclk = [%d] \n", __func__, current_band_for_pclk);
	// s3cfb_change_pclk_mode(current_band_for_pclk, fbdev);
#endif

#ifdef CONFIG_FB_S3C_MDNIE
#if 1 // !(defined(CONFIG_MACH_P1) && defined(CONFIG_TARGET_LOCALE_KOR))
	...
	pr_info("[LCD] s3cfb_late_resume is complete\n");
	return ;
}
