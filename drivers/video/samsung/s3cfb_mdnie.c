...
static char banner[] __initdata = KERN_INFO "[LCD] S3C MDNIE Driver, (c) 2010 Samsung Electronics\n";
...
int s3c_mdnie_hw_init(void)
{
    ...
    if(s3c_mdnie_base == NULL) {
        printk(KERN_ERR "[LCD] MDNIE failed ioremap\n");
        return -ENOENT;
    }
	...
	if (IS_ERR(mdnie_clock)) {
        printk("[LCD] failed to get mdnie clock source\n");
		return -EINVAL;
	}

    printk("[LCD] MDNIE  INIT SUCCESS Addr : 0x%p\n",s3c_mdnie_base);
	return 0;
}
...
int s3c_mdnie_init_global(struct s3cfb_global *s3cfb_ctrl)
{
	...
	printk("[LCD] s3c_mdnie_init_global :: mDNIe_Mode_Change to mDNIe_UI \n");
	mDNIe_Mode_Change(mDNIe_UI);
#else
