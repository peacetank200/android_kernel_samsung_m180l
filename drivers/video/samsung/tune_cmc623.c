...
// P1(M180S) VOC : sometimes, not predictable, White line is happened at right-top position.
#define FEATURE_CMC623_PCLK_8mA
...
static bool cmc623_I2cWrite16( unsigned char Addr, unsigned long Data)
{
	...
	printk("[LCD] %s i2c transfer error:%d(a:%x)\n", __func__, err, Addr);/* add by inter.park */
	return err;    
}
...
static int cmc623_I2cRead16_direct(u8 reg, u16 *val, int isDirect)
{
	...
	printk(KERN_ERR "[LCD] %s %d i2c transfer error: %d\n", __func__, __LINE__, err);/* add by inter.park */
...
#ifdef FEATURE_CMC623_PCLK_8mA
	cmc623_I2cWrite16(0x16, 0x155D);
#else // src 
...
{
	printk(KERN_INFO "[LCD] %s called\n", __func__);
		
	if(!p_cmc623_data)
	{
		printk(KERN_ERR "[LCD] %s cmc623 is not initialized\n", __func__);
		return 0;
	}	
