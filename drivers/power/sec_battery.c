...
#if 1 /* External Charger IC is used */
	/* if SMB136 charger has detected USBIN */
	if ((chg->pdata && chg->pdata->external_charger &&
		chg->pdata->external_charger->get_connection_status &&
		chg->pdata->external_charger->get_connection_status())
		||(chg->pdata && chg->pdata->pmic_charger &&
...
static bool check_samsung_charger(void)
{
	...
	mdelay(5); //Wait for completing to switch.
...
static int sec_bat_charging_control(struct chg_data *chg)
{
	...
	int ret;
	static int nReChkCnt=3;   

	/* Check if the current TA is really an improper TA during boot-up time*/
	if (chg->bat_info.batt_improper_ta == true
		&& chg->cable_status == CABLE_TYPE_AC 
		&& nReChkCnt > 0 ){  
		
		prev_cable_status = CABLE_TYPE_NONE;
		pr_info("%s: Improper charger rechecking[%d]\n", __func__,nReChkCnt);
		nReChkCnt--;
	}
