...
void enter_upload_mode(unsigned long val)
{
	...
	//printk(KERN_DEBUG "enter_upload_mode val = %d , kernel_sec_get_debug_level() = ....\n", val, kernel_sec_get_debug_level());
	...
	//printk(KERN_DEBUG "%s - uploadmode %d \n", __func__, uploadmode);

	if(uploadmode)
	{
		//printk(KERN_DEBUG "%s - kernel_sec_viraddr_wdt_reset_reg = %d\n", kernel_sec_viraddr_wdt_reset_reg);
...
static void gpio_keys_report_event(struct gpio_button_data *bdata)
{
	...
	int state = (gpio_get_value(button->gpio) ? 1 : 0) ^ button->active_low;
		...
		if(KERNEL_SEC_DEBUG_LEVEL_LOW == kernel_sec_get_debug_level())
		{
			printk(KERN_WARNING "[Key] KERNEL_SEC_DEBUG_LEVEL_LOW\n");
			//force user upload
			if(sec_debug_check_crash_key(button->code, state))
			{
				//printk(KERN_WARNING "[Key] enter user force upload\n");
				enter_upload_mode(1);
			}
		}
        else {
    ...
    if(state)
    {
        ...
        printk(KERN_DEBUG "[Key] %s key is pressed.\n", button->desc);
#endif
    }
    else
    {
        ...
        {
#ifdef FEATURE_KERNEL_INPUT_DEBUG_MSG                
            printk(KERN_DEBUG "[Key] %s key is released.\n", button->desc);
#endif
        }
        else
        {
            ...
            input_sync(input);
#ifdef FEATURE_KERNEL_INPUT_DEBUG_MSG                    
            printk(KERN_DEBUG "[Key] %s key - workaround. \n", button->desc);
#endif
            msleep(1);
        }
