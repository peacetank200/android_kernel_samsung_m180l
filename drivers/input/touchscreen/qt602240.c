...
#ifdef FEATURE_KERNEL_INPUT_DEBUG_MSG        
...
#ifdef CONFIG_KOR_MODEL_M180L
    keyarray_config.tchthr = 20;
...
//TS_DEBUG_DATA_READ_FROM_NODE_POSITION
static uint8_t set_refer_position = 0;
static uint8_t set_delta_position = 0;

static ssize_t set_refer_position_mode_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    unsigned int position;
    int ret;

    if (sscanf(buf, "%u", &position) != 1) {
    	dev_err(dev, "Invalid values\n");
    	return -EINVAL;
    }

    if (position < 0 || position > 215) {
    	dev_err(dev, "Invalid values\n");
    	return -EINVAL;        
    }
    set_refer_position = (uint8_t)position;

    return size;
}

static ssize_t set_delta_position_mode_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    unsigned int position;
    int ret;
    ...
    set_delta_position = (uint8_t)position;

    return size;
}


static ssize_t set_refer_position_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    uint16_t qt_refrence=0;
    read_dbg_data(QT_REFERENCE_MODE, set_refer_position, &qt_refrence);
    return sprintf(buf, "%u\n", qt_refrence);
}

static ssize_t set_delta_position_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    uint16_t qt_delta=0;
    read_dbg_data(QT_DELTA_MODE, set_delta_position,&qt_delta);
    if(qt_delta < 32767)
        return sprintf(buf, "%u\n", qt_delta);
    else
        qt_delta = 65535 - qt_delta;
...
//TS_DEBUG_DATA_READ_FROM_NODE_POSITION
static DEVICE_ATTR(set_delta_position, S_IRUGO | S_IWUSR, set_delta_position_mode_show, set_delta_position_mode_set);
static DEVICE_ATTR(set_refer_position, S_IRUGO | S_IWUSR, set_refer_position_mode_show, set_refer_position_mode_set);
		...
		const struct i2c_device_id *id)
{
    ...
    //TS_DEBUG_DATA_READ_FROM_NODE_POSITION
    if (device_create_file(qt602240_noise_test, &dev_attr_set_delta_position)< 0)
        printk(KERN_ERR "Failed to create device file(%s)!\n", dev_attr_set_delta_position.attr.name);
    if (device_create_file(qt602240_noise_test, &dev_attr_set_refer_position) < 0)
        printk(KERN_ERR "Failed to create device file(%s)!\n", dev_attr_set_refer_position.attr.name);
#endif
