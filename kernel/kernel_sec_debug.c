...
/* for "user force upload" */
extern bool sec_battery_get_jig_status(void);
...
void kernel_sec_set_upload_magic_number(void)
{
	...
	{
		*magic_virt_addr = LOKE_BOOT_USB_DWNLDMAGIC_USER_NO;	
...
//	Ap_Cp_Switch_Config(0);
	...
	if (bSilentReset || (KERNEL_SEC_DEBUG_LEVEL_LOW == kernel_sec_get_debug_level() && UPLOAD_CAUSE_FORCED_UPLOAD != gkernel_sec_upload_cause))
	{
...
int sec_debug_check_crash_key(unsigned int code, int value)
{
	static enum { NONE, STEP1, STEP2, STEP3, STEP4, STEP5, STEP6, STEP7, STEP8, STEP9, STEP10} state = NONE;

	//int *magic_virt_addr = (int*) LOKE_BOOT_USB_DWNLD_V_ADDR;
	//printk("%s : magic code = %x \n", __func__, *magic_virt_addr);
	switch (state) 
	{
	case NONE:
		state = (code == LOCKUP_FIRST_KEY && value) ? STEP1 : NONE;
		//pr_info("sec_debug_check_crash_key step none. %d\n",state);
		break;
	case STEP1:
		state = (code == LOCKUP_EXTRA_KEY && value) ? STEP2 : NONE;
		//pr_info("sec_debug_check_crash_key step STEP1. %d\n",state);
		break;
	case STEP2:
		state = (code == LOCKUP_FIRST_KEY && !value) ? STEP3 : NONE;
		//pr_info("sec_debug_check_crash_key step STEP2.%d\n",state);
		break;
	case STEP3:
		state = (code == LOCKUP_FIRST_KEY && value) ? STEP4 : NONE;
		//pr_info("sec_debug_check_crash_key step STEP3. %d\n",state);
		break;
	case STEP4:
		state = (code == LOCKUP_FIRST_KEY && !value) ? STEP5 : NONE;
		//pr_info("sec_debug_check_crash_key step STEP4. %d\n",state);
		break;
	case STEP5:
		state = (code == LOCKUP_FIRST_KEY && value) ? STEP6 : NONE;
		//pr_info("sec_debug_check_crash_key step STEP5. %d\n",state);
		break;
	case STEP6:
		state = (code == LOCKUP_EXTRA_KEY && !value) ? STEP7 : NONE;
		//pr_info("sec_debug_check_crash_key step STEP6. %d\n",state);
		break;
	case STEP7:
	...
	case STEP8:
		...
		pr_info("sec_debug_check_crash_key step STEP8. %d\n",state);
		break;
	case STEP9:
		state = (code == LOCKUP_EXTRA_KEY && value) ? STEP10 : NONE;
		pr_info("sec_debug_check_crash_key step STEP9. %d\n",state);
		break;
	case STEP10:
		pr_info("sec_debug_check_crash_key step STEP10. %d\n",state);
		if (code == LOCKUP_THIRD_KEY && value) 
		{
			pr_info("sec_battery_get_jig_status() = %d\n", sec_battery_get_jig_status());
			
			if (sec_battery_get_jig_status())
				return 1;
		}					
		else
			state = NONE;
...
EXPORT_SYMBOL(sec_debug_check_crash_key);
