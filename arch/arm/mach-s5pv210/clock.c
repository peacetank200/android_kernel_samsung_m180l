...
void __init_or_cpufreq s5pv210_setup_clocks(void)
{
	...
	printk(KERN_DEBUG "[CLK] %s: registering clocks\n", __func__);

	clkdiv0 = __raw_readl(S5P_CLK_DIV0);
	clkdiv1 = __raw_readl(S5P_CLK_DIV1);

	printk(KERN_DEBUG "[CLK] %s: clkdiv0 = %08x, clkdiv1 = %08x\n", __func__, clkdiv0, clkdiv1);
	...
	printk(KERN_DEBUG "[CLK] %s: xtal is %ld\n", __func__, xtal);
	...
	printk(KERN_INFO "[CLK] S5PV210: PLL settings, A=%ld, M=%ld, E=%ld V=%ld", apll, mpll, epll, vpll);
	...
	printk(KERN_INFO "[CLK] S5PV210: ARMCLK=%ld, HCLKM=%ld, HCLKD=%ld\n" "HCLKP=%ld, PCLKM=%ld, PCLKD=%ld, PCLKP=%ld\n", armclk, hclk_msys, hclk_dsys, hclk_psys,	pclk_msys, pclk_dsys, pclk_psys);
	...
	for (ptr = 0; ptr < ARRAY_SIZE(clksrcs); ptr++) {
		...
		} else if (!strcmp(pclkSrc->clk.name, "sclk_cam") && (pclkSrc->clk.id == 0)) {
