...
int s3cfb_clk_on(struct platform_device *pdev, struct clk **s3cfb_clk)
{
	...
	if (IS_ERR(sclk)) {
		dev_err(&pdev->dev, "[LCD] failed to get sclk for fimd\n");
		goto err_clk1;
	}

#if defined(CONFIG_S5PV210_SCLKFIMD_USE_VPLL)
	mout_mpll = clk_get(&pdev->dev, "sclk_vpll");
	dev_dbg(&pdev->dev, "[LCD] s3cfb_clk_on :: Use sclk_vpll\n");
#else
	mout_mpll = clk_get(&pdev->dev, "mout_mpll");
	dev_dbg(&pdev->dev, "[LCD] s3cfb_clk_on :: Use mout_mpll\n");
#endif

	if (IS_ERR(mout_mpll)) {
		dev_err(&pdev->dev, "[LCD] failed to get mout_mpll\n");
		goto err_clk2;
	}
	...
	dev_dbg(&pdev->dev, "[LCD] set fimd sclk rate to %d\n", rate);
	...
	if (IS_ERR(sclk_mdnie)) {
		...
		#if defined(CONFIG_TARGET_PCLK_44_46)
		printk(KERN_INFO "[LCD] s3cfb_clk_on :: [45 Mhz]");
		clk_set_rate(sclk_mdnie, 45*1000000);
		#elif defined(CONFIG_TARGET_PCLK_47_6)
		printk(KERN_INFO "[LCD] s3cfb_clk_on :: [48 Mhz]");
		clk_set_rate(sclk_mdnie, 48*1000000);
		#else
		printk(KERN_INFO "[LCD] s3cfb_clk_on :: [54 Mhz]");
	...
	if (IS_ERR(sclk_mdnie_pwm)) {
		dev_err(&pdev->dev, "[LCD] failed to get sclk for mdnie pwm\n");
		}
	else
		{
		clk_xusb = clk_get(&pdev->dev, "xusbxti");
		if (IS_ERR(clk_xusb)) {
			dev_err(&pdev->dev, "[LCD] failed to get xusbxti for mdnie pwm\n");
			}
		else
			{
...
int s3cfb_backlight_onoff(struct platform_device *pdev, int onoff)
{
	...
	if (err) {
		printk(KERN_ERR "failed to request GPD0 for " "lcd backlight control\n");
		return err;
	}
