...
{
	wm8994_set_off,
	wm8994_set_videocall_receiver,
	wm8994_set_videocall_speaker,
	wm8994_set_videocall_headset,
	wm8994_set_videocall_headphone,
	wm8994_set_videocall_bluetooth
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if(wm8994->codec_state & CALL_ACTIVE)
	{
		...
		if (wm8994->codec_state & (VOIP_CALL_ACTIVE)) //kks_110621
		    s5pv210_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_3);

		wm8994->codec_state &= ~(VOICE_CALL_ACTIVE|VIDEO_CALL_ACTIVE|VOIP_CALL_ACTIVE);		
#else
...
static int wm8994_set_videocall_path(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct wm8994_priv *wm8994 = codec->drvdata;
	...
	// Get path value
	int path_num = ucontrol->value.integer.value[0];
	DEBUG_LOG("wm8994_set_videocall_path param[%d] wm8994->cur_path[%d] wm8994->codec_state[%d]", path_num,wm8994->cur_path,wm8994->codec_state);
	...
	switch(path_num)
	{
		...
		case HP_NO_MIC:	
		case BT :
			DEBUG_LOG("routing video path to %s", mc->texts[path_num] );
			break;
	...
	if((wm8994->cur_path != path_num) || !(wm8994->codec_state & CALL_ACTIVE))
	{
  	...
  	DEBUG_LOG("wm8994_set_videocall_path 2");
		wm8994->codec_state |= CALL_ACTIVE;
		wm8994->codec_state |= (VIDEO_CALL_ACTIVE);
		wm8994->cur_path = path_num;
		wm8994->universal_videocall_path[wm8994->cur_path](codec);

	}
	else
	{
  	DEBUG_LOG("wm8994_set_videocall_path 3");
		val = wm8994_read(codec, WM8994_AIF1_DAC1_FILTERS_1);
		val &= ~(WM8994_AIF1DAC1_MUTE_MASK);
		val |= (WM8994_AIF1DAC1_UNMUTE);
		wm8994_write(codec, WM8994_AIF1_DAC1_FILTERS_1, val);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	switch(control_data)
	{
		...
		case CMD_CODEC_EMERGENCY_RECOVERY:
			...
			//
        		DEBUG_LOG("[WM8994] CMD_CODEC_EMERGENCY_RECOVERY");
			...
			DEBUG_LOG("[WM8994] CMD_CODEC_EMERGENCY_RECOVERY done.");
			break;
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if(wm8994->cur_path != path_num || !(wm8994->codec_state & CALL_ACTIVE))
	{
		wm8994->codec_state |= CALL_ACTIVE;
		wm8994->codec_state |= (VOIP_CALL_ACTIVE);
		s5pv210_lock_dvfs_high_level(DVFS_LOCK_TOKEN_3, L1);
		wm8994->cur_path = path_num;
...
static int configure_clock(struct snd_soc_codec *codec)
{
	...
	//kth_101128 block for fixing vtcall tik noise[
