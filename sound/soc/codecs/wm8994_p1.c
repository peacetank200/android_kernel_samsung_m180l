...
// Video Call SPK
//#define TUNING_VTCALL_SPK_INPUTMIX_VOL          0x10 // 18h // [DH08-0005] 0x10(+7.5dB)->0x0a(-1.5dB) // NXP Req.
#define TUNING_VTCALL_MAINMIC_INPUTMIX_VOL      0x11 //kth_101123  0x0a->0x11 18h
#define TUNING_VTCALL_SUBMIC_INPUTMIX_VOL       0x011 //kth_101125  0x0a->0x11 18h

/***  Onbaord MIC  ***/
// Normal rec. : main mic
#define TUNING_RECORD_MAIN_INPUTMIX_VOL         0x16 // 18h // [DF14-2230] 0x18(+19.5dB)->0x16(+16.5dB) // H/W Req.
#else
#define TUNING_VTCALL_MAINMIC_INPUTMIX_VOL      0x11 //kth_101123  0x0a->0x11 18h
#define TUNING_VTCALL_SUBMIC_INPUTMIX_VOL       0x011 //kth_101125  0x0a->0x11 18h
#endif
...
// Video Call
#define TUNING_VTCALL_EAR_INPUTMIX_VOL          0x1F // 1Ah
...
// Call Ear MIC
#define  TUNING_CALL_MAIN_INPUTMIX_VOL 0x0B //kth_101125
#define TUNING_CALL_EAR_INPUTMIX_VOL 0x0B //kth_101125 0x0B<-0x1F 1Ah
...
static short int  video_call_setpath; //kth_101122
int audio_init(void)
{
	//CODEC LDO SETTING
...
{
    int val, ch=0;
  	DEBUG_LOG(" close_rx_path sel[%d]",sel);
    switch (sel)
    {
        case PLAYBACK_OFF : 
...
void wm8994_disable_playback_path(struct snd_soc_codec *codec, enum playback_path path)
{
	...
	DEBUG_LOG("wm8994_disable_playback_path = [%d]", path);
	...
	switch(rec_path)
	{
		case MAIN:
			...
			if(!(wm8994->codec_state & CALL_ACTIVE))	// Normal case
			{
				...
				val &= ~(WM8994_AIF1ADC1R_ENA_MASK|WM8994_ADCR_ENA_MASK|WM8994_ADCL_ENA_MASK |WM8994_AIF1ADC1L_ENA_MASK); //kth_101025
...
void wm8994_record_headset_mic(struct snd_soc_codec *codec) 
{
        ...
        DEBUG_LOG("wm8994_record_headset_mic HWREV[%d] wm8994->codec_state[%d]",HWREV,wm8994->codec_state);
...
#endif        
        {
                ...
                val = wm8994_read(codec,WM8994_POWER_MANAGEMENT_1 );
                val &= ~(WM8994_VMID_SEL_MASK|WM8994_BIAS_ENA_MASK|WM8994_SPKOUTR_ENA_MASK | WM8994_SPKOUTL_ENA_MASK); //kth_101025
        ...
        else
        {
                ...
                val = wm8994_read(codec, WM8994_POWER_MANAGEMENT_1);
                val &= ~(WM8994_VMID_SEL_MASK|WM8994_BIAS_ENA_MASK); //kth_101025
                ...
                wm8994_write(codec, WM8994_POWER_MANAGEMENT_2, val);
            		...
            		{	
                        ...
                        if(wm8994->codec_state & VIDEO_CALL_ACTIVE)
                        {
                            val |= (WM8994_IN1R_VU|TUNING_VTCALL_EAR_INPUTMIX_VOL);
                        }
                        else
                        {
                        if(wm8994->recognition_active == REC_ON)
        ...
        val |= (WM8994_ADCR_ENA | WM8994_AIF1ADC1R_ENA); //kth_101025
        wm8994_write(codec,WM8994_POWER_MANAGEMENT_4 ,val);
...
void wm8994_videophone_onboard_mic(struct snd_soc_codec *codec)
{
	...
	DEBUG_LOG("+++");
       ...
       printk("[WM8994] Recording Main Mic - normal Call\n");
        ...
        {   
            ...
            if(wm8994->codec_state & VIDEO_CALL_ACTIVE)
            {
                if (wm8994->cur_path == SPK)
                    val |= (WM8994_IN1L_VU|0xb); //kks_101128 TUNING_VTCALL_MAINMIC_INPUTMIX_VOL->0xb
                else 
                    val |= (WM8994_IN1L_VU|0xb); //kks_101128 TUNING_VTCALL_SUBMIC_INPUTMIX_VOL->0xb
            }
            else
            {
                if(wm8994->recognition_active == REC_ON)
...
void wm8994_videophone_headset_mic(struct snd_soc_codec *codec) 
{
        struct wm8994_priv *wm8994 = codec->drvdata;
        ...
        DEBUG_LOG("Recording through Headset Mic 3 wm8994->codec_state[%d]",wm8994->codec_state);
        // Mixing left channel output to right channel.
        val = wm8994_read(codec, WM8994_AIF1_CONTROL_1);	//605H : 0x0010
        val &= ~(WM8994_AIF1ADCL_SRC_MASK | WM8994_AIF1ADCR_SRC_MASK);
        val |= (WM8994_AIF1ADCL_SRC | WM8994_AIF1ADCR_SRC);
...
void wm8994_voip_onboard_mic(struct snd_soc_codec *codec)
{
	struct wm8994_priv *wm8994 = codec->drvdata;
	...
	DEBUG_LOG("+++");
       ...
       printk("[WM8994] Recording Main Mic - normal Call\n");
        ...
        {   
            ...
            val = wm8994_read(codec, WM8994_LEFT_LINE_INPUT_1_2_VOLUME);
            val &= ~(WM8994_IN1L_MUTE_MASK|WM8994_IN1L_VOL_MASK);
						printk("[WM8994] codecstate %x\n", wm8994->codec_state);
            if(wm8994->codec_state & VOIP_CALL_ACTIVE)
            {
							printk("[WM8994] codecstate 2 %x\n", wm8994->codec_state);

                val |= (WM8994_IN1L_VU|0x1f); //kks_101128 TUNING_VTCALL_SUBMIC_INPUTMIX_VOL->0xb
            }
						else
            {
                if(wm8994->recognition_active == REC_ON)
...
void wm8994_voip_headset_mic(struct snd_soc_codec *codec) 
{
        struct wm8994_priv *wm8994 = codec->drvdata;
	...
	{	
        ...
        val = wm8994_read(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME);
        val &= ~(WM8994_IN1R_MUTE_MASK|WM8994_IN1R_VOL_MASK);
        if(wm8994->codec_state & (VIDEO_CALL_ACTIVE|VOIP_CALL_ACTIVE)) // jhyukbyun voip)
        {
            val |= (WM8994_IN1R_VU|TUNING_VTCALL_EAR_INPUTMIX_VOL);
        }
        else
...
void wm8994_record_onboard_mic(struct snd_soc_codec *codec)
{
	...
	if(wm8994->codec_state & VOICE_CALL_ACTIVE)
	{
		...
		val |= (WM8994_AIF1ADC1L_ENA|WM8994_ADCL_ENA); //kth_101025
		wm8994_write(codec, WM8994_POWER_MANAGEMENT_4, val);
    ...
    else if((wm8994->codec_state & VIDEO_CALL_ACTIVE))
    {
        printk("[WM8994] Recording Main Mic - normal Call\n");
        ...
        {   
            ...
            if(wm8994->codec_state & VIDEO_CALL_ACTIVE)
            {
                if (wm8994->cur_path == RCV)
                    val |= (WM8994_IN1L_VU|TUNING_VTCALL_MAINMIC_INPUTMIX_VOL);
                else 
                    val |= (WM8994_IN1L_VU|TUNING_VTCALL_SUBMIC_INPUTMIX_VOL);
            }
            else
            {
                if(wm8994->recognition_active == REC_ON)
...
void wm8994_record_main_mic(struct snd_soc_codec *codec) 
{
	...
	DEBUG_LOG("wm8994_record_main_mic");
	audio_ctrl_mic_bias_gpio(1);
...
void wm8994_record_bluetooth(struct snd_soc_codec *codec)
{
	...
	DEBUG_LOG("wm8994_record_bluetooth wm8994->codec_state[%d]",wm8994->codec_state);
...
//kth_101123 make function wm8994_set_videophone_playback_headset because of vcall incoming pop noise
void wm8994_set_videophone_playback_headset(struct snd_soc_codec *codec)
{
	struct wm8994_priv *wm8994 = codec->drvdata;
	...
	if(wm8994->testmode_config_flag == SEC_NORMAL)
	{
		...
		val |= (WM8994_HPOUT1L_MUTE_N | 0x39); //kth_110115
		else
		val |= (WM8994_HPOUT1L_MUTE_N | TUNING_MP3_OUTPUTL_VOL);
		wm8994_write(codec, WM8994_LEFT_OUTPUT_VOLUME, 0x00);


		val = wm8994_read(codec,WM8994_RIGHT_OUTPUT_VOLUME);
		val &= ~(WM8994_HPOUT1R_MUTE_N_MASK | WM8994_HPOUT1R_VOL_MASK);
		if(wm8994->ringtone_active)
		val |= (WM8994_HPOUT1_VU | WM8994_HPOUT1R_MUTE_N | 0x39); //kth_110115
		else
		val |= (WM8994_HPOUT1_VU | WM8994_HPOUT1R_MUTE_N | 0x39); //kth_110115
	...
	//Enable Charge Pump	
	val = wm8994_read(codec, WM8994_CHARGE_PUMP_1);
	if(val != (WM8994_CP_ENA | WM8994_CP_ENA_DEFAULT))
	{
	    val &= ~WM8994_CP_ENA_MASK ;
...
void wm8994_set_videophone_playback_speaker(struct snd_soc_codec *codec)
{
#ifdef CONFIG_TARGET_LOCALE_KOR
	struct wm8994_priv *wm8994 = codec->drvdata;
#endif
...
void wm8994_set_voip_playback_headset(struct snd_soc_codec *codec)
{
	struct wm8994_priv *wm8994 = codec->drvdata;
       ...
       if(wm8994->testmode_config_flag == SEC_NORMAL)
	{
		...
		val &= ~(WM8994_HPOUT1L_MUTE_N_MASK | WM8994_HPOUT1L_VOL_MASK);
		if(wm8994->ringtone_active)
		val |= (WM8994_HPOUT1L_MUTE_N | 0x33); // jhyuk.byun 101228 TUNING_RING_OUTPUTL_VOL);
		else
		val |= (WM8994_HPOUT1L_MUTE_N | 0x33); // jhyuk.byun 101228 TUNING_MP3_OUTPUTL_VOL);
		wm8994_write(codec, WM8994_LEFT_OUTPUT_VOLUME, val);
		...
		val &= ~(WM8994_HPOUT1R_MUTE_N_MASK | WM8994_HPOUT1R_VOL_MASK);
		if(wm8994->ringtone_active)
		val |= (WM8994_HPOUT1_VU | WM8994_HPOUT1R_MUTE_N | 0x33); // jhyuk.byun 101228 TUNING_RING_OUTPUTR_VOL);
		else
		val |= (WM8994_HPOUT1_VU | WM8994_HPOUT1R_MUTE_N | 0x33); // jhyuk.byun 101228 TUNING_MP3_OUTPUTR_VOL);
		wm8994_write(codec, WM8994_RIGHT_OUTPUT_VOLUME, val);
        }
}

void wm8994_set_voip_playback_speaker(struct snd_soc_codec *codec)
{
#ifdef CONFIG_TARGET_LOCALE_KOR
	struct wm8994_priv *wm8994 = codec->drvdata;
#endif
...
void wm8994_set_playback_headset(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec,WM8994_CLASS_W_1,val);
       ...
       //kth_110527 fixed by geunwook.kang
       //- Fixed headset playback gain when changing path from lineout playback path on ULP running.
       //[Bug_Description_ENGLISH]
       //-Problem: Difference headset playback gain when changing path from lineout playback path on ULP running.
       //-Cause: Do not reset codec gain when changing playback path.
       //-Solution: Reset codec gain when setting headset playback path.

     	wm8994_write(codec,WM8994_OUTPUT_MIXER_5, 0x0);
     	wm8994_write(codec,WM8994_OUTPUT_MIXER_6, 0x0);
	// Headset Control
	if(wm8994->testmode_config_flag == SEC_NORMAL)
	{
		val = wm8994_read(codec,WM8994_LEFT_OUTPUT_VOLUME);
...
void wm8994_set_playback_speaker_headset(struct snd_soc_codec *codec)
{
	...
	DEBUG_LOG("wm8994_set_playback_speaker_headset wm8994->codec_state[%d]",wm8994->codec_state);
	//------------------  Common Settings ------------------
	// Enable the Timeslot0 to DAC1L
	val = wm8994_read(codec, WM8994_DAC1_LEFT_MIXER_ROUTING	);	   
	val &= ~(WM8994_AIF1DAC1L_TO_DAC1L_MASK);
...
void wm8994_set_playback_bluetooth(struct snd_soc_codec *codec)
{
	...
	DEBUG_LOG("wm8994_set_playback_bluetooth");
...
void wm8994_set_voicecall_headphone(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, 0x03, 0x0000); // MIXOUTL/R_VOL

	/*Digital Path Enables and Unmutes*/	
	wm8994_write(codec, WM8994_DAC1_LEFT_MIXER_ROUTING, 0x0001); // [AP input]
	wm8994_write(codec, WM8994_DAC1_RIGHT_MIXER_ROUTING, 0x0001); // [AP input]

	/*Analogue Input Configuration*/
	wm8994_write(codec, WM8994_POWER_MANAGEMENT_2, 0x6340); // MIXINL/R,IN1L enable [main mic]
	...
	if(!wm8994->testmode_config_flag)
	{
		...
		val |= (WM8994_IN1L_VU|TUNING_CALL_MAIN_INPUTMIX_VOL); //kth_101125
		wm8994_write(codec, WM8994_LEFT_LINE_INPUT_1_2_VOLUME, val);
		...
		wm8994_write(codec, WM8994_INPUT_MIXER_6, 0x0002); // IN2LRP_MIXINR_VOL -12dB //kks_101110_1 0x0001->0x0002
	}

	wm8994_write(codec, WM8994_INPUT_MIXER_2, 0x0030); // IN1LN/P_TO_IN1L [main mic]

	// code add
	// 3-POLE Path
	wm8994_write(codec, 0x34, 0x0002); // IN1L_TO_LINEOUT1P(diff.) [CP input]

	wm8994_write(codec, 0x1E, 0x0006); // LINEOUT1N/P On, LINEOUT1_VOL 0dB(MAX) [CP input]
	//~code add
	...
	if(!wm8994->testmode_config_flag)
	{
		val = wm8994_read(codec, WM8994_LEFT_OPGA_VOLUME);
		val &= ~(WM8994_MIXOUTL_MUTE_N_MASK|WM8994_MIXOUTL_VOL_MASK);
		val |= (WM8994_MIXOUT_VU|WM8994_MIXOUTL_MUTE_N|TUNING_CALL_OPGAL_VOL); // MIXOUTL enable
		wm8994_write(codec, WM8994_LEFT_OPGA_VOLUME, val);

		val = wm8994_read(codec, WM8994_RIGHT_OPGA_VOLUME);
		val &= ~(WM8994_MIXOUTR_MUTE_N_MASK|WM8994_MIXOUTR_VOL_MASK);
		val |= (WM8994_MIXOUT_VU|WM8994_MIXOUTR_MUTE_N|TUNING_CALL_OPGAR_VOL); // MIXOUTR enable
		wm8994_write(codec, WM8994_RIGHT_OPGA_VOLUME, val);
	}
	...
	if(!wm8994->testmode_config_flag)
	{
		...
		val |= (WM8994_HPOUT1L_MUTE_N|TUNING_CALL_OUTPUTL_VOL); // [headset out]
		wm8994_write(codec, WM8994_LEFT_OUTPUT_VOLUME, val);

		val = wm8994_read(codec, WM8994_RIGHT_OUTPUT_VOLUME);
		val &= ~(WM8994_HPOUT1R_MUTE_N_MASK|WM8994_HPOUT1R_VOL_MASK);
		val |= (WM8994_HPOUT1_VU|WM8994_HPOUT1R_MUTE_N|TUNING_CALL_OUTPUTR_VOL); // [headset out]
		wm8994_write(codec, WM8994_RIGHT_OUTPUT_VOLUME, val);
	}
	...
	wm8994_write(codec, 0x01, 0x0303); // VMID, MICbias enable
	wm8994_write(codec, 0x60, 0x0022); // HPOUTL/R intermediate stage
	wm8994_write(codec, 0x4C, 0x9F25); // charge pump

	msleep(5);

	wm8994_write(codec, 0x05, 0x0303); // [AP input]

	/*Analogue Output Configuration*/	
	wm8994_write(codec, WM8994_OUTPUT_MIXER_1, 0x0081); // MIXINR_TO_MIXOUTL
	wm8994_write(codec, WM8994_OUTPUT_MIXER_2, 0x0041); // MIXINR_TO_MIXOUTR

	wm8994_write(codec, 0x03, 0x3030); // LINEOUTL/R,MIXOUTL/R enable

	wm8994_write(codec, 0x54, 0x0303); // DCservo HPOUT enable/ DC offset enable
	...
	wm8994_write(codec, 0x60, 0x00EE); // HPOUT1L/R enable [ear outpath]

	//Unmute DAC1 left
	val = wm8994_read(codec, WM8994_DAC1_LEFT_VOLUME);
	val &= ~(WM8994_DAC1L_MUTE_MASK|WM8994_DAC1L_VOL_MASK);
	val |= (WM8994_DAC1_VU|TUNING_DAC1L_VOL); // DAC1L volume 0db(Not mute)
	wm8994_write(codec, WM8994_DAC1_LEFT_VOLUME, val);

	//Unmute and volume ctrl RightDAC
	val = wm8994_read(codec, WM8994_DAC1_RIGHT_VOLUME);
	val &= ~(WM8994_DAC1R_MUTE_MASK|WM8994_DAC1R_VOL_MASK);
	val |= (WM8994_DAC1_VU|TUNING_DAC1R_VOL); // DAC1R volume 0db(Not mute)
	wm8994_write(codec, WM8994_DAC1_RIGHT_VOLUME, val);

    val = wm8994_read(codec, WM8994_AIF1_CONTROL_2); 
    val &= ~(WM8994_AIF1DAC_BOOST_MASK);
    val |= (0x00<<WM8994_AIF1DAC_BOOST_SHIFT); // AIF1 Boost +12dB	//kkuram 101201
    wm8994_write(codec, WM8994_AIF1_CONTROL_2, val);

    val = wm8994_read(codec, WM8994_AIF2_CONTROL_2); 
    val &= ~(WM8994_AIF1DAC_BOOST_MASK);
    val |= (0x00<<WM8994_AIF2DAC_BOOST_SHIFT); // AIF2 Boost 0dB
    wm8994_write(codec, WM8994_AIF2_CONTROL_2, val);

	wm8994_write(codec, WM8994_AIF1_DAC1_FILTERS_1, 0x0000); // 420h //
	printk(KERN_DEBUG "[WM8994] wm8994_set_voicecall_headphone() ---\n");
...
void wm8994_set_voicecall_headset(struct snd_soc_codec *codec)
{
	...
	if(!wm8994->testmode_config_flag)
	{
		...
		wm8994_write(codec, WM8994_INPUT_MIXER_3, val); // [mic path]
#endif 

		// unmute right pga, set volume 
		val = wm8994_read(codec, WM8994_INPUT_MIXER_4);
		val&= ~(WM8994_IN1R_TO_MIXINR_MASK|WM8994_IN1R_MIXINR_VOL_MASK|WM8994_MIXOUTR_MIXINR_VOL_MASK);
		val |= (WM8994_IN1R_TO_MIXINR); //  [ear mic path]
		wm8994_write(codec, WM8994_INPUT_MIXER_4, val); 

		//wm8994_write(codec, WM8994_INPUT_MIXER_6, 0x0000); // 2Ch // [DE03-1723] // IN2LRP_MIXINR_VOL 0x03(-6dB)
		wm8994_write(codec, WM8994_INPUT_MIXER_5, 0x0002); // IN2LRP_MIXINL_VOL -12dB //kks_101110_1 0x0001->0x0002
	}

    if((wm8994->testmode_config_flag == SEC_TEST_15MODE)
        ||(wm8994->testmode_config_flag ==SEC_TEST_HQRL_LOOPBACK)
        || (wm8994->testmode_config_flag ==SEC_TEST_PBA_LOOPBACK)) //kth_101110 kbg_101029 kbg_101105
	{
		val = wm8994_read(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME);
		val &= ~(WM8994_IN1R_MUTE_MASK|WM8994_IN1R_VOL_MASK);
		val |= (WM8994_IN1R_VU|TUNING_CALL_EAR_INPUTMIX_VOL);
		...
		wm8994_write(codec, WM8994_INPUT_MIXER_5, 0x0002); // IN2LRP_MIXINL_VOL -12d //kks_101110_1 0x0001->0x0002
	}

	wm8994_write(codec, WM8994_INPUT_MIXER_2, 0x0003); // IN1RN_TO_IN1R [ear mic path]

	// code add
	wm8994_write(codec, 0x34, 0x0004); // IN1R_TO_LINEOUT1P(diff) [cradle path]

	wm8994_write(codec, 0x1E, 0x0006); // WM8994_LINE_OUTPUTS_VOLUME // LINEOUT1N/P On, LINEOUT1_VOL 0dB(MAX) [cradle path]
	//~code add
	...
	if(!wm8994->testmode_config_flag)
	{
		...
		wm8994_write(codec, WM8994_RIGHT_OUTPUT_VOLUME, val); // [headset path]
	}
	...
	wm8994_write(codec, 0x01, 0x0303); // HPOUT1N/R,bias enable
	wm8994_write(codec, 0x60, 0x0022); // HPOUT1L/R intermediate stage
	...
	wm8994_write(codec, 0x03, 0x3030); // LINEOUTL/R_ENA, MIXOUTL/R_ENA [CP path]

	wm8994_write(codec, 0x54, 0x0303); // DC servo
	...
	wm8994_write(codec, 0x60, 0x00EE); // Analogue HP
    ...
    val = wm8994_read(codec, WM8994_AIF1_CONTROL_2); 
    val &= ~(WM8994_AIF1DAC_BOOST_MASK);
    val |= (0x00<<WM8994_AIF1DAC_BOOST_SHIFT); // 00 = 0dB, 01 = +6dB, 02 = +12dB, 03 = +18dB	//kkuram 101201
    wm8994_write(codec, WM8994_AIF1_CONTROL_2, val);
	...
	wm8994_write(codec, WM8994_AIF1_DAC1_FILTERS_1, 0x0000); // 420h //
	printk(KERN_DEBUG "[WM8994] wm8994_set_voicecall_headset() ---\n");
   ...
   // AIF2 ADC Filters 
   wm8994_write(codec,0x510, 0x3800 );
#endif // trock@101125 prime voicecall headset tuning
...
void wm8994_set_voicecall_speaker(struct snd_soc_codec *codec)
{
	...
	/*Analogue Input Configuration*/
	// code add
	wm8994_write(codec, WM8994_POWER_MANAGEMENT_2, 0x6340); //INL[mic],mixinL/mixinR, themal sensor enable
	//~code add
	wm8994_write(codec, WM8994_INPUT_MIXER_2, 0x0030); // IN1N_TO_IN1L, IN1LP_TO_IN1L
	...
	/*Analogue Output Configuration*/
	// code add
	wm8994_write(codec, WM8994_LINE_MIXER_1, 0x0002); // IN1L_TO_LINEOUT1P(diff) [mic path]
       wm8994_write(codec, WM8994_LINE_OUTPUTS_VOLUME, 0x0006); // LINEOUT1N/P [mic path]
       wm8994_write(codec, WM8994_INPUT_MIXER_6, 0x0005); // IN2LRP_MIXINR_VOL[2:0] = b101 // RxVoice to MIXINR [CP outpath]? 
	...
	if(!wm8994->testmode_config_flag)
	{
	 ...
	 wm8994_write(codec, 0x2D, 0x0081); // MIXINR_TO_MIXOUTL, DAC1L_TO_MIXOUTL [AP outpath + CP outpath]
	 wm8994_write(codec, 0x2E, 0x0041); // MIXINR_TO_MIXOUTR, DAC1R_TO_MIXOUTR [AP outpath + CP outpath]
	//~code add
       wm8994_write(codec, 0x03, 0x33F0); // LINEOUT1N/P, SPKl/RVOL, MIXOUTL/RVOL, MIXOUTL/R enable
	wm8994_write(codec, 0x05, 0X0303); // AIF1DAC1L/R, DAC1L/R enable AP outpath
	...
	if(!wm8994->testmode_config_flag)
	{
		...
		val = wm8994_read(codec, WM8994_SPEAKER_VOLUME_LEFT);
		val &= ~(WM8994_SPKOUTL_MUTE_N_MASK|WM8994_SPKOUTL_VOL_MASK);
		val |= (WM8994_SPKOUT_VU|WM8994_SPKOUTL_MUTE_N|TUNING_CALL_SPKL_VOL); // SPKLOUT enable
		wm8994_write(codec, WM8994_SPEAKER_VOLUME_LEFT, val);

		val = wm8994_read(codec, WM8994_SPEAKER_VOLUME_RIGHT);
		val &= ~(WM8994_SPKOUTR_MUTE_N_MASK|WM8994_SPKOUTR_VOL_MASK);
		val |= (WM8994_SPKOUT_VU|WM8994_SPKOUTR_MUTE_N|TUNING_CALL_SPKR_VOL); // SPKROUT enable
		wm8994_write(codec, WM8994_SPEAKER_VOLUME_RIGHT, val);

		val = wm8994_read(codec, WM8994_CLASSD);
		val &= ~(WM8994_SPKOUTL_BOOST_MASK|WM8994_SPKOUTR_BOOST_MASK);  //kks_101031	  val &= ~(WM8994_SPKOUTL_BOOST_MASK);
		val |= TUNING_CALL_CLASSD_VOL; //kks_101031       val |= (TUNING_CALL_CLASSD_VOL<<WM8994_SPKOUTL_BOOST_SHIFT);
		wm8994_write(codec, WM8994_CLASSD, val);
	}
	...
	val |= (WM8994_SPKMIXL_TO_SPKOUTL|WM8994_SPKMIXR_TO_SPKOUTR); // SPKMIXL/R_TO_SPKOUTL/R On [Two spkout]
	wm8994_write(codec, WM8994_SPKOUT_MIXERS, val);
	//~code add

	/* Digital Path Enables and Unmutes*/
	wm8994_write(codec, WM8994_AIF1_DAC1_FILTERS_1, WM8994_AIF1DAC1_UNMUTE); // kbg_101109
	...
	wm8994_write(codec, WM8994_DC_SERVO_1, 0x0000); // DC Servo 1
	wm8994_write(codec, WM8994_POWER_MANAGEMENT_1, WM8994_SPKOUTL_ENA|WM8994_SPKOUTR_ENA|WM8994_VMID_SEL_NORMAL|WM8994_BIAS_ENA); //kks_101101
    ...
    if((wm8994->testmode_config_flag == SEC_TEST_15MODE)
        ||(wm8994->testmode_config_flag == SEC_TEST_HQRL_LOOPBACK)
        ||(wm8994->testmode_config_flag == SEC_TEST_PBA_LOOPBACK) //kth_101110
    )
    {
        // Unmute the SPKMIXVOLUME kbg_101105
        val = wm8994_read(codec, WM8994_SPKMIXL_ATTENUATION);
        val &= ~(WM8994_SPKMIXL_VOL_MASK);
        val |= TUNING_SPKMIXL_ATTEN;    
        wm8994_write(codec, WM8994_SPKMIXL_ATTENUATION, val);
        ...
        val |= (WM8994_SPKMIXL_TO_SPKOUTL|WM8994_SPKMIXR_TO_SPKOUTR);
        wm8994_write(codec,WM8994_SPKOUT_MIXERS,val );      
    printk(KERN_DEBUG "[WM8994] wm8994_set_voicecall_speaker() SEC_TEST_HQRL_LOOPBACK---\n");
    }
	...
	printk(KERN_DEBUG "[WM8994] wm8994_set_voicecall_speaker() P1LGU---\n");
#elif defined(CONFIG_TARGET_LOCALE_KOR)
...
void wm8994_set_voicecall_bluetooth(struct snd_soc_codec *codec)
{
    ...
    wm8994_write(codec, WM8994_AIF2_MASTER_SLAVE, 0x7000); // AIF2 Master mode, BCLK2 enable, LRCLK2 enable
    wm8994_write(codec, WM8994_AIF1_CONTROL_2, 0x4000); // AIF1 Left DAC recives right interface data	//kkuram 101201

    /* Analog Output Power Management */
    wm8994_write(codec, 0x01, 0x0003); //?
    wm8994_write(codec, 0x02, 0x6200); // MIXINL enable

	/* Digital Path Enables and Unmutes */
    wm8994_write(codec, 0x04, 0x2002); // AIF2ADCL, ADCL enable
    wm8994_write(codec, 0x05, 0x220A); // AIF2DACL, AIF1DAC1L, DAC2L, DAC1L enable
    ...
    // CP analog path
    wm8994_write(codec, 0x2B, 0x0003); // RXVOICE(VRXN/VRXP) -9dB //kks_101110_1 0x0002->0x0003
    wm8994_write(codec, 0x1E, 0x0010); // LINEOUT1 volume 0dB

    wm8994_write(codec, 0x20, 0x0179); // MIXOUTL enable, 0dB,
    wm8994_write(codec, 0x21, 0x0139); // MIXOUTR disable, 0dB,

    wm8994_write(codec, 0x34, 0x0001); // MIXOUTL_TO_LINEOUT1P  [CP input]
    ...
    wm8994_write(codec, WM8994_POWER_MANAGEMENT_6, 0x000A); // AIF3->AIF2ADCDAT2, AIF2DACDAT ->DACDAT3 select

	// AIF1 & AIF2 Output is connected to DAC1	
    wm8994_write(codec, 0x600, 0x0000); // WM8994_DAC1_MIXER_VOLUMES

    wm8994_write(codec, 0x601, WM8994_AIF2DACL_TO_DAC1L); // AIF2DACL_TO_DAC1L
    wm8994_write(codec, 0x602, 0x0000);

    wm8994_write(codec, 0x603, 0x000C); // ADCL_DAC2_VOL, 0dB
    ...
    /* Analog Output Power Management */
    wm8994_write(codec, WM8994_LINE_MIXER_1, 0x01); // MIXOUTL_TO_LINEOUT1P by Single-Ended
    wm8994_write(codec, WM8994_POWER_MANAGEMENT_3, 0x3020); // LINEOUT1N/P, MIXOUTLVOL, MIXOUTL enable

	//Unmute DAC1 left
	val = wm8994_read(codec, WM8994_DAC1_LEFT_VOLUME);
	val &= ~(WM8994_DAC1L_MUTE_MASK|WM8994_DAC1L_VOL_MASK);
	val |= (TUNING_DAC1L_VOL); // DAC1L 0dB on
    wm8994_write(codec, 0x610, val);
	
	//Unmute and volume ctrl RightDAC
	val = wm8994_read(codec, WM8994_DAC1_RIGHT_VOLUME); 
	val &= ~(WM8994_DAC1R_MUTE_MASK|WM8994_DAC1R_VOL_MASK);
	val |= (TUNING_DAC1R_VOL); // DAC1R 0dB on
    wm8994_write(codec, 0x611, val);

    // Enable DAC1L to HPOUT1L path
	wm8994_write(codec, WM8994_OUTPUT_MIXER_1, WM8994_DAC1L_TO_MIXOUTL); // DAC1L_TO_MIXOUTL

	wm8994_write(codec, WM8994_AIF1_DAC1_FILTERS_1, 0x0000);  
	wm8994_write(codec, WM8994_AIF2_DAC_FILTERS_1, 0x0080);

	printk(KERN_DEBUG "[WM8994] wm8994_set_voicecall_bluetooth() ---\n");
...
void wm8994_set_voicecall_record(struct snd_soc_codec *codec, int channel)
{
        ...
        DEBUG_LOG("wm8994_set_voicecall_record channel = [%d] wm8994 ->call_record_path[%d]", channel,wm8994 ->call_record_path);
        ...
        if(wm8994 ->call_record_path == CALL_RECORDING_MAIN )
        {
                ...
                {
	                // while voicecall recording, headset unplugged => speaker rx mute
	                val = wm8994_read(codec, WM8994_INPUT_MIXER_4);
	                val &= ~( WM8994_IN1R_MIXINR_VOL_MASK | WM8994_MIXOUTR_MIXINR_VOL_MASK);
	                wm8994_write(codec, WM8994_INPUT_MIXER_4, val); 
	                ...
	                val = wm8994_read(codec, WM8994_INPUT_MIXER_3);
	                val &= ~(WM8994_IN1L_TO_MIXINL_MASK | WM8994_IN1L_MIXINL_VOL_MASK | WM8994_MIXOUTL_MIXINL_VOL_MASK);
	                val |= (WM8994_IN1L_TO_MIXINL | WM8994_IN1L_MIXINL_VOL|0x07); //30dB, 6dB
	                wm8994_write(codec, WM8994_INPUT_MIXER_3, val);  				
                }
#endif				
                switch(channel)
                {
                        case CH_UDLINK:  //TxRx
                                ...
                                wm8994_write(codec,0x400,0x01ef);	//kth_101125 AIF1 INPUT VOLUME : 0x00C0:0db
                        break;
        ...
        else if(wm8994 ->call_record_path == CALL_RECORDING_SUB)
        {
                ...
                {
	                // while voicecall recording, headset plugged => headset rx mute
	                val = wm8994_read(codec, WM8994_INPUT_MIXER_3);
	                val &= ~(WM8994_IN1L_MIXINL_VOL_MASK | WM8994_MIXOUTL_MIXINL_VOL_MASK);
	                wm8994_write(codec, WM8994_INPUT_MIXER_3, val); // [mic path]

	                // headset voicecall call recording rx volume up
	                val = wm8994_read(codec, WM8994_INPUT_MIXER_4);
	                val &= ~(WM8994_IN1R_TO_MIXINR_MASK | WM8994_IN1R_MIXINR_VOL_MASK | WM8994_MIXOUTR_MIXINR_VOL_MASK);
	                val |= (WM8994_IN1R_TO_MIXINR | WM8994_IN1R_MIXINR_VOL | 0x07); //30dB, 6dB // record rx + 6dB
	                wm8994_write(codec, WM8994_INPUT_MIXER_4, val); 
                }
#endif 
                ...
                switch(channel)
                {
                        case CH_UDLINK:  //TxRx
                                ...
                                val &= ~(WM8994_AIF1ADC1R_ENA_MASK|WM8994_ADCR_ENA_MASK|WM8994_ADCL_ENA_MASK |WM8994_AIF1ADC1L_ENA_MASK); //kth_101025
                                //val |= ( WM8994_AIF1ADC1L_ENA|WM8994_AIF1ADC1R_ENA);
                                val |= (WM8994_AIF1ADC1R_ENA|WM8994_ADCR_ENA); //kth_101025
                                wm8994_write(codec,WM8994_POWER_MANAGEMENT_4,val);	

                                wm8994_write(codec,0x300,0xC010);	

                                wm8994_write(codec,0x401,0x01ef);	//kkuram_101125	// AIF1 INPUT VOLUME : 0x00C0:0db	
                        break;
                        ...
                        default:
                                ...
                                val &= ~(WM8994_ADCR_ENA_MASK |WM8994_AIF1ADC1R_ENA_MASK );
                                //val |= (WM8994_AIF1ADC1R_ENA | WM8994_ADCR_ENA  );
                                val |= (WM8994_AIF1ADC1R_ENA|WM8994_ADCR_ENA_MASK); //kth_101025
                                wm8994_write(codec,WM8994_POWER_MANAGEMENT_4 ,val);
...
void wm8994_call_recording_change_path(struct snd_soc_codec *codec)
{
        ...
        DEBUG_LOG("wm8994_call_recording_change_path rec_path[%d] -> call_record_path [%d]", wm8994 ->rec_path, wm8994 ->call_record_path);           
...
void wm8994_set_videocall_receiver(struct snd_soc_codec *codec)
{
    ...
    wm8994_set_playback_receiver(codec);
    //wm8994_record_main_mic(codec);
    wm8994_record_onboard_mic(codec); 
	audio_ctrl_mic_bias_gpio(1);
    
    printk(KERN_DEBUG "[WM8994] wm8994_set_videocall_receiver() ---\n");
}

void wm8994_set_videocall_headset(struct snd_soc_codec *codec)
{
    DEBUG_LOG("+++");
   //kth_101123 fix vcall incoming pop noise     close_rx_path(codec, OFF);
    wm8994_set_videophone_playback_headset(codec); //kth_101123
    wm8994_videophone_headset_mic(codec); //kth_101123 make wm8994_videophone_onboard_mic
 
    //audio_ctrl_mic_bias_gpio(1);
    
    printk(KERN_DEBUG "[WM8994] wm8994_set_videocall_headset() ---\n");
}
...
void wm8994_set_videocall_headphone(struct snd_soc_codec *codec)
{
    ...
    wm8994_videophone_onboard_mic(codec); //kth_101123 make wm8994_videophone_onboard_mic
	audio_ctrl_mic_bias_gpio(1);
    
    printk(KERN_DEBUG "[WM8994] wm8994_set_videocall_headphone() ---\n");
}

void wm8994_set_videocall_speaker(struct snd_soc_codec *codec)
{
	DEBUG_LOG("+++");
    close_rx_path(codec, OFF);

    wm8994_set_videophone_playback_speaker(codec);
    //wm8994_record_sub_mic(codec);
    wm8994_videophone_onboard_mic(codec); //kth_101123 make wm8994_videophone_onboard_mic
	audio_ctrl_mic_bias_gpio(1);
    
    printk(KERN_DEBUG "[WM8994] wm8994_set_videocall_speaker() ---\n");
}

void wm8994_set_videocall_bluetooth(struct snd_soc_codec *codec)
{
    struct wm8994_priv *wm8994 = codec->drvdata;
    ...
    close_rx_path(codec, OFF);
    /*initialize register*/
    wm8994_write(codec, 0x3, 0x0);
    wm8994_write(codec, 0x1c, 0x6d);
    wm8994_write(codec, 0x1d, 0x6d);
    wm8994_write(codec, 0x22, 0x3);
    wm8994_write(codec, 0x23, 0x3);
    wm8994_write(codec, 0x25, 0x140);
    wm8994_write(codec, 0x26, 0x79);
    wm8994_write(codec, 0x27, 0x79);
    wm8994_write(codec, 0x2d, 0x0);
    wm8994_write(codec, 0x2e, 0x0);
    wm8994_write(codec, 0x36, 0x0);
    wm8994_write(codec, 0x4c, 0x1f25);
    wm8994_write(codec, 0x51, 0x4);
    wm8994_write(codec, 0x54, 0x0);
    wm8994_write(codec, 0x55, 0x54a);
    wm8994_write(codec, 0x57, 0x0);
    wm8994_write(codec, 0x60, 0x0);
    wm8994_write(codec, 0x480, 0x6318);
    wm8994_write(codec, 0x482, 0xfca);
    wm8994_write(codec, 0x483, 0x400);
    wm8994_write(codec, 0x484, 0xd8);
    wm8994_write(codec, 0x491, 0x564);
    wm8994_write(codec, 0x492, 0x559);
    wm8994_write(codec, 0x493, 0x4000);
    wm8994_write(codec, 0x601, 0x0);
    wm8994_write(codec, 0x602, 0x0);
    wm8994_write(codec, 0x610, 0x2c0);
    wm8994_write(codec, 0x611, 0x2c0);
    wm8994_write(codec, 0x730, 0x500);
    wm8994_write(codec, 0x731, 0x11e0);
	...
	wm8994_write(codec, WM8994_AIF2_CONTROL_1, 0x4118); // DSP Mode, BCLK invert, LRCLK normal
    wm8994_write(codec, WM8994_AIF2_BCLK, 0x40);
    wm8994_write(codec, WM8994_AIF2_CONTROL_2, 0x4400); //kth_101123 0x4000 -> 0x4400 // Left & Right DAC receives left interface data
    ...
    printk(KERN_DEBUG "[WM8994] wm8994_set_videocall_bluetooth() ---\n");
}
#endif
...
{	
	...
	DEBUG_LOG("wm8994_set_fmradio_common onoff = [%d]", onoff);
...
{	
	...
	DEBUG_LOG("wm8994_set_fmradio_headset Routing ear path : FM Radio -> EAR Out");
...
void wm8994_set_fmradio_speaker(struct snd_soc_codec *codec)
{
	...
	DEBUG_LOG("wm8994_set_fmradio_speaker Routing spk path : FM Radio -> SPK Out");
...
DEBUG_LOG("+++");
    //close_rx_path(codec, OFF);
    wm8994_set_voip_playback_speaker(codec);
    //wm8994_record_sub_mic(codec);
    wm8994_voip_onboard_mic(codec); //kth_101123 make wm8994_videophone_onboard_mic

    audio_ctrl_mic_bias_gpio(1);
    
    printk(KERN_DEBUG "[WM8994] wm8994_set_voipcall_speaker() ---\n");
}
...
void wm8994_set_voipcall_headset(struct snd_soc_codec *codec)
{
    ...
    DEBUG_LOG("+++");
   //kth_101123 fix vcall incoming pop noise     close_rx_path(codec, OFF);
    wm8994_set_voip_playback_headset(codec); //kth_101123
    wm8994_voip_headset_mic(codec); //kth_101123 make wm8994_videophone_onboard_mic
    
    printk(KERN_DEBUG "[WM8994] wm8994_set_voipcall_headset() ---\n");
}
